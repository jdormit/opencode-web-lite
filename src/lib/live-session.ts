  ...
  for (const event of events) {
    ...
    if (event.type === 'message.updated') {
      ...
      if (existing) {
        ...
        const createdAt = typeof created === 'number' && Number.isFinite(created) ? created : Date.now()
        next.items.push({
          id,
          role,
          createdAt,
    ...
    if (event.type === 'message.removed') {
      ...
      if (messageID) next.items = next.items.filter((item) => item.id !== messageID)
      continue
    }
    if (event.type === 'message.part.delta') {
      ...
      else message.parts.push({ id: partID, type: 'text', text: delta })
      continue
    }
...
function projectPart(part: Record<string, unknown>): SessionSnapshot['items'][number]['parts'][number] | undefined {
  ...
  if (type === 'text' && typeof part.text === 'string') {
    return { id, type: 'text', text: part.text.slice(0, 100_000) }
  }
  if (type === 'tool') {
